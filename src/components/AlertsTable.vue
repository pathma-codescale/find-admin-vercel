...
const emit = defineEmits<{
    ...
    view: [alertId: string | number, isViewed: boolean]
...
const totalPages = computed(() =>
    Math.ceil(filteredAlerts.value.length / props.itemsPerPage)
)
...
const handleView = (alertId: string | number, isViewed: boolean) => {
    emit('view', alertId, isViewed)
}
...
<template>
    <div class="w-full bg-white dark:bg-gray-800 z-10  border border-borderGray rounded-xl overflow-hidden">
        <!-- Header Section -->
        <div
            class="flex items-center bg-white dark:bg-gray-800  justify-between pl-4 px-2 py-1 sm:p-2 border-b border-borderGray">
            <h2 class="text-black dark:text-white text-md font-bold sm:text-lg ml-2 lg:text-lg font-semibold">
                {{ $t('alertsTable.tableHeader') }}</h2>
        ...
        <div class="divide-y divide-border-borderGray">
            ...
            <div v-else>
                ...
                <div class="hidden lg:block">
                    <div v-for="alert in paginatedAlerts" :key="alert.alertId"
                        ...
                        <div class="col-span-2 flex flex-col gap-1">
                            <span v-for="(ch, idx) in getChannels(alert.channels)" :key="idx"
                        ...
                        <div class="col-span-1 flex items-center justify-center space-x-1">
                            <ActionButtonSet :item-id="alert.alertId" :is-banned="alert.isBanned"
                                :is-viewed="alert.isViewed" @ban="handleBan" @view="handleView" @edit="handleEdit"
                                @delete="handleDelete" />
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-between p-4 border-t border-borderGray">
                <div class="text-black dark:text-white text-sm">
                    {{ $t('pagination.showing', { start: (currentPage - 1) * itemsPerPage + 1, total: totalPages }) }}
                </div>

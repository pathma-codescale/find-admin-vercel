...
const sortOptions = [
  ...
  { id: 'name-asc', label: t('filters.sort.nameAZ') },
]

// Status options
const statusOptions = [
  { id: 'all', label: t('filters.status.all') },
  { id: 'active', label: t('filters.status.active') },
  { id: 'suspended', label: t('filters.status.suspended') },
...
const toggleStatus = (status: string) => {
  if (status === 'all') {
    // If 'All Candidates' is selected, clear other selections
    localFilters.status = localFilters.status.includes('all') ? [] : ['all']
  } else {
    // Remove 'all' if it's selected
    if (localFilters.status.includes('all')) {
      localFilters.status = localFilters.status.filter((s) => s !== 'all')
    }
    ...
    <div ref="modalRef" class="w-full max-w-md bg-gray-800 rounded-lg shadow-xl overflow-hidden">
      ...
      <div class="flex justify-between items-center p-4 border-b border-gray-700">
        <div class="flex items-center">
          ...
          <h3 class="text-lg font-medium text-white">{{ t('filters.title') }}</h3>
        </div>
      ...
      <div class="p-4 max-h-[70vh] overflow-y-auto">
        ...
        <div class="mb-6">
          <div class="flex items-center mb-3">
            ...
            <h4 class="text-md font-medium text-white">{{ t('filters.filterByStatus') }}</h4>
          </div>
